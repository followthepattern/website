function ForwardIcon(props: {className: string}) {
  return (
...
export function ForwardButton({ player, amount = 10 }: {player: any, amount?: number}) {
  return (
    <button
      type="button"
      className="relative rounded-full group focus:outline-none"
      onClick={() => player.seekBy(amount)}
      aria-label={`Fast-forward ${amount} seconds`}
    >
      <div className="absolute -inset-4 -left-2 md:hidden" />
      <ForwardIcon className="w-6 h-6 stroke-blue-500 group-hover:stroke-blue-700" />
    </button>
  )
}

...
const playbackRates = [
  {
    value: 1,
    icon: function PlaybackIcon(props: {className: string}) {
      return (
        <svg
          aria-hidden="true"
          viewBox="0 0 16 16"
...
export function PlaybackRateButton({ player }: {player: any}) {
  ...
  return (
    <button
      type="button"
      className="relative flex items-center justify-center w-6 h-6 text-blue-500 rounded-md hover:bg-slate-100 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2"
      onClick={() => {
    ...
    >
      ...
      <playbackRate.icon className="w-4 h-4" />
    </button>
  )
}
